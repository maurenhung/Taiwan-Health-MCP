...
class FHIRMedicationService:
    ...
    def create_medication(
        ...
        try:
            ...
            medication = {
                ...
                "manufacturer": {
                    "display": drug_info.get("manufacturer", "")
                },
            ...
            if drug_info.get("valid_date"):
                medication["batch"] = {
                    "expirationDate": drug_info["valid_date"]
                }
    ...
    def create_medication_knowledge(
        ...
        try:
            ...
            if drug_info.get("indication"):
                med_knowledge["indication"] = [{
                    "text": drug_info["indication"]
                }]
            ...
            if drug_info.get("package"):
                med_knowledge["packaging"] = {
                    "type": {
                        "text": drug_info["package"]
                    }
                }

            # 7. 加入藥品類別（處方/非處方）
            if drug_info.get("category"):
                med_knowledge["drugCharacteristic"] = med_knowledge.get("drugCharacteristic", [])
                med_knowledge["drugCharacteristic"].append({
                    "type": {
                        "text": "藥品類別"
                    },
                    "valueCodeableConcept": {
                        "text": drug_info["category"]
                    }
                })
    ...
    def _get_appearance(self, license_id: str) -> List[Dict]:
        ...
        if appearance.get("shape"):
            extensions.append({
                "url": "shape",
                "valueString": appearance["shape"]
            })

        if appearance.get("color"):
            extensions.append({
                "url": "color",
                "valueString": appearance["color"]
            })

        if appearance.get("marking"):
            extensions.append({
                "url": "marking",
                "valueString": appearance["marking"]
            })
    ...
    def _generate_medication_id(self, license_id: str) -> str:
        """生成 Medication ID"""
        # 移除特殊字元
        clean_id = license_id.replace("衛署", "").replace("衛部", "").replace("字第", "").replace("號", "")

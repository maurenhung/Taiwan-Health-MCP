...
class ClinicalGuidelineService:
    ...
    def _initialize_database(self):
        ...
        try:
            ...
            # 建立索引
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_icd_code ON disease_guidelines(icd_code)")
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_disease_name ON disease_guidelines(disease_name_zh)")
    ...
    def _populate_taiwan_guidelines(self, cursor):
        ...
        # 診斷建議
        diagnostic_steps = [
            (1, "實驗室檢查", "空腹血糖 ≥126 mg/dL，或隨機血糖 ≥200 mg/dL 合併典型症狀，或HbA1c ≥6.5%", "A"),
        ...
        # 用藥建議
        medications = [
            ("第一線", "雙胍類 (Biguanide)", "Metformin", "起始劑量 500mg 每日一次，逐漸增加至 500-1000mg 每日兩次", "腎功能不全 (eGFR <30)", "A"),
            ("第二線", "SGLT2 抑制劑", "Empagliflozin, Dapagliflozin", "依藥品仿單建議劑量", "eGFR <20-30 (依藥品而異)", "A"),
            ("第二線", "GLP-1 受體促效劑", "Dulaglutide, Semaglutide", "皮下注射，每週一次", "甲狀腺髓樣癌病史或家族史", "A"),
            ("第二線", "DPP-4 抑制劑", "Sitagliptin, Linagliptin", "依藥品仿單建議劑量", "無特殊禁忌", "B"),
            ("輔助治療", "胰島素", "長效/速效胰島素", "依血糖控制情況調整劑量", "需注意低血糖風險", "A"),
        ]
        ...
        # 檢查建議
        tests = [
            ("生化檢驗", "糖化血色素 (HbA1c)", "4548-4", "每3個月", "評估血糖控制", "A"),
            ("生化檢驗", "空腹血糖", "1558-6", "每次回診", "監測血糖", "A"),
            ("生化檢驗", "腎功能 (Cr, eGFR)", "2160-0", "每年至少1次", "篩檢糖尿病腎病變", "A"),
            ("尿液檢驗", "尿液白蛋白/肌酸酐比值", "14959-1", "每年至少1次", "篩檢早期腎病變", "A"),
            ("生化檢驗", "血脂肪 (TC, LDL, HDL, TG)", "2093-3", "每年至少1次", "評估心血管風險", "A"),
        ...
        # 治療目標
        goals = [
            ("血糖控制", "HbA1c", "<7%（一般成人），<8%（老年人或多重共病）", "長期控制目標"),
        ...
        cursor.execute("""
            ...
            (guideline_id, step_order, recommendation_type, description, evidence_level)
            VALUES (?, ?, ?, ?, ?)
        """, (guideline_id, 1, "血壓測量", "診間血壓 ≥140/90 mmHg，或家庭血壓 ≥135/85 mmHg", "A"))

        cursor.execute("""
            INSERT INTO diagnostic_recommendations
            (guideline_id, step_order, recommendation_type, description, evidence_level)
            VALUES (?, ?, ?, ?, ?)
        """, (guideline_id, 2, "次發性高血壓篩檢", "年輕患者（<40歲）或突發性高血壓應排除次發性原因", "B"))

        # 用藥建議
        hypertension_meds = [
            ("第一線", "血管收縮素轉換酶抑制劑 (ACEI)", "Enalapril, Ramipril", "起始低劑量，逐步調整", "孕婦、雙側腎動脈狹窄", "A"),
            ("第一線", "血管收縮素受體阻斷劑 (ARB)", "Losartan, Valsartan", "起始低劑量，逐步調整", "孕婦", "A"),
            ("第一線", "鈣離子通道阻斷劑 (CCB)", "Amlodipine, Nifedipine", "長效型為佳", "心臟傳導異常", "A"),
            ("第一線", "利尿劑", "Hydrochlorothiazide", "低劑量使用（12.5-25mg）", "痛風、低血鉀", "A"),
            ("第二線", "乙型阻斷劑 (Beta-blocker)", "Bisoprolol, Carvedilol", "有心臟病或年輕患者優先考慮", "氣喘、心搏過慢", "B"),
        ]
        ...
        # 檢查建議
        hypertension_tests = [
            ("生化檢驗", "腎功能 (Cr, eGFR)", "2160-0", "每年至少1次", "評估腎臟損害", "A"),
            ("生化檢驗", "電解質 (Na, K)", "2951-2", "開始利尿劑治療前及定期追蹤", "監測電解質異常", "A"),
            ("心電圖", "靜態心電圖", None, "診斷時及每年追蹤", "評估心臟肥大或缺血", "B"),
        ...
        cursor.execute("""
            ...
            (guideline_id, goal_type, target_parameter, target_value, timeframe)
            VALUES (?, ?, ?, ?, ?)
        """, (guideline_id, "血壓控制", "血壓", "<140/90 mmHg（一般成人），<130/80 mmHg（糖尿病或慢性腎臟病患者）", "長期目標"))
        ...
        # 用藥建議
        lipid_meds = [
            ("第一線", "史他汀類 (Statin)", "Atorvastatin, Rosuvastatin", "中至高強度，依心血管風險決定", "活動性肝病、孕婦", "A"),
            ("第二線", "Ezetimibe", "Ezetimibe", "10mg 每日一次，可與 Statin 併用", "無特殊禁忌", "B"),
            ("第二線", "PCSK9 抑制劑", "Evolocumab, Alirocumab", "皮下注射，用於高風險且 Statin 無法達標者", "成本考量", "A"),
            ("其他", "纖維酸類 (Fibrate)", "Fenofibrate", "主要用於高三酸甘油酯", "腎功能不全", "B"),
        ]
        ...
        cursor.execute("""
            ...
            (guideline_id, test_category, test_name, loinc_code, frequency, indication, evidence_level)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (guideline_id, "生化檢驗", "血脂肪 (TC, LDL, HDL, TG)", "2093-3", "開始治療前及治療4-12週後追蹤", "評估治療效果", "A"))

        # 治療目標
        lipid_goals = [
            ("血脂控制", "LDL-C", "<100 mg/dL（中風險），<70 mg/dL（高風險），<55 mg/dL（極高風險）", "依心血管風險分層"),
            ("血脂控制", "三酸甘油酯", "<150 mg/dL", "降低心血管風險"),
            ("血脂控制", "HDL-C", ">40 mg/dL（男性），>50 mg/dL（女性）", "心血管保護因子"),
        ]
    ...
    def suggest_clinical_pathway(self, icd_code: str, patient_context: Optional[Dict] = None) -> str:
        """
        根據疾病建議臨床路徑
        ...
        clinical_pathway = {
            ...
            "pathway": {
                "step1_diagnosis": {
                    "phase": "診斷確認階段",
                    "actions": [rec["description"] for rec in guideline_data["diagnostic_recommendations"]]
                },
